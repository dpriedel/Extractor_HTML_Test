...
const EM::FileName FILE_WITH_HTML_10Q{
...
const EM::FileName FILE_WITH_XML2_10Q{
    "/vol_DA/SEC/Archives/edgar/data/1341319/0001445866-13-001158.txt"};
...
const EM::FileName FILE_WITH_HTML_10Q_WITH_ANCHORS{
    "/vol_DA/SEC/Archives/edgar/data/1420525/0001420525-09-000028.txt"};
const EM::FileName FILE_WITH_HTML_10Q_WITH_ANCHORS2{
    "/vol_DA/SEC/Archives/edgar/data/1434743/0001193125-09-235665.txt"};
const EM::FileName FILE_WITH_HTML_10Q_WITH_ANCHORS3{
    "/vol_DA/SEC/Archives/edgar/data/1326688/0001104659-09-064933.txt"};
const EM::FileName FILE_WITH_HTML_10Q_WITH_ANCHORS4{
    "/vol_DA/SEC/Archives/edgar/data/1466739/0001002014-13-000458.txt"};
const EM::FileName FILE_WITH_HTML_10Q_WITH_ANCHORS5{
    "/vol_DA/SEC/Archives/edgar/data/1460602/0001062993-13-005017.txt"};
...
const EM::FileName FILE_WITH_HTML_10Q_MINIMAL_DATA{
...
const EM::FileName FILE_WITH_HTML_10Q_PROBLEM_REGEX2{
    "/vol_DA/SEC/Archives/edgar/data/4515/0000004515-13-000053.txt"};
const EM::FileName FILE_WITH_HTML_10Q_PROBLEM_WITH_ASSETS1{
    "/vol_DA/SEC/Archives/edgar/data/68270/0000068270-13-000059.txt"};
const EM::FileName FILE_WITH_NO_HTML_10Q{
    "/vol_DA/SEC/SEC_forms/0000855931/10-Q/0001130319-01-500242.txt"};
const EM::FileName FILE_WITH_BAD_CASH_FLOW_VALUE{
    "/vol_DA/SEC/Archives/edgar/data/949982/0001477932-13-004674.txt"};

const EM::FileName FILE_WITH_NO_HTML_AND_NO_FILENAME_10K{
    "/vol_DA/SEC/SEC_forms/0001000015/10-K/0000912057-00-014793.txt"};

const EM::FileName FILE_WITH_NO_HTML2_10Q{
    "/vol_DA/SEC/Archives/edgar/data/1421907/0001165527-13-000854.txt"};
const EM::FileName FILE_WITH_ANCHOR_LOOP{
    "/vol_DA/SEC/SEC_forms/0000758938/10-K/0000950124-06-005605.txt"};
...
const EM::FileName FILE_WITH_HTML_10Q_FIND_SHARES1{
    "/vol_DA/SEC/Archives/edgar/data/29989/0000029989-13-000015.txt"};
const EM::FileName FILE_WITH_HTML_NO_HREFS1_10K{
    "/vol_DA/SEC/SEC_forms/0000906345/10-K/0000906345-04-000036.txt"};
const EM::FileName FILE_WITH_HTML_ANCHORS_10Q{
    "/home/dpriedel/projects/Extractor_Markup/YUM_bad_balsheet.html"};
...
const EM::FileName TRAINING_FILE_FOR_SHARES_OUTSTANDING{
    "/home/dpriedel/projects/Extractor_HTML_Test/"
    "shares_outstanding_training_file.txt"};

const EM::FileName FILE_WITH_SHARES_AND_VALUE{
    "/vol_DA/SEC/Archives/edgar/html/0000004127/"
    "0000004127-15-000037.txt_fy1510k1022015.htm"};
const EM::FileName FILE_WITH_NO_POSSIBLES{
    "/vol_DA/SEC/Archives/edgar/html/0001609711/"
    "0001609711-19-000012.txt_a201810-kx10kdocument.htm"};
const EM::FileName FILE_WITH_SHARES_BEFORE_YESNO{
    "/vol_KUtil2/SEC_exports/html/0000019617/10-K/"
    "0000950123-04-002022.txt_y94051e10vk.htm"};
const EM::FileName FILE_WITH_SHARES_AFTER_CONTENTS{
    "/vol_KUtil2/SEC_exports/html/0000000020/10-K/"
    "0000893220-05-000728.txt_w06061e10vk.htm"};
const EM::FileName FILE_WITH_SHARE_VALUE_NO_DOLLAR_SIGN{
    "/vol_KUtil2/SEC_exports/html/0001326160/10-K/"
    "0001326160-14-000003.txt_form10k.htm"};
const EM::FileName FILE_WITH_XBRL_INSIDE_HTM_DOCUMENT{
    "/vol_KUtil2/SEC_exports/html/0000029905/10-K/"
    "0000029905-19-000019.txt_wfx-20181231.htm"};
const EM::FileName FILE_WITH_XBRL_INSIDE_TXT_DOCUMENT{
    "/vol_DA/SEC/SEC_forms/0000029905/10-K/0000029905-19-000019.txt"};
...
TEST_F(Iterators, AnchorRangeFileWithXML_10Q) {
  ...
  int total =
      rng::accumulate(htmls | rng::views::transform([](const auto &html) {
...
TEST_F(LocateDocumentWithFinancialContent,
       FindContentInFileHasMinimalData_10Q) {
  const auto file_content_10Q =
...
TEST_F(FindAnchorsForFinancialStatements, FindSegmentedTopLevelAnchor_10Q) {
  const auto file_content_10Q =
      LoadDataFileForUse(FILE_WITH_HTML_10Q_WITH_SEGMENTED_ANCHORS);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);
  ...
  auto found_some = rng::adjacent_find(
      anchors, [](const auto &a, const auto &b) { return a.href_ == b.href_; });
...
TEST_F(FindAnchorsForFinancialStatements, FindTopLevelAnchor_10Q) {
  ...
  auto financial_content = FindFinancialContentUsingAnchors(
      &sections, FILE_WITH_HTML_10Q_WITH_ANCHORS);

  ASSERT_TRUE(financial_content);
}

TEST_F(FindAnchorsForFinancialStatements,
       FindTopLevelAnchorInFileWithMinimalHTML_10Q) {
  const auto file_content_10Q =
      LoadDataFileForUse(FILE_WITH_HTML_10Q_MINIMAL_DATA);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);
  ...
  auto financial_content = FindFinancialContentUsingAnchors(
      &sections, FILE_WITH_HTML_10Q_MINIMAL_DATA);

  ASSERT_TRUE(financial_content);
}

TEST_F(FindAnchorsForFinancialStatements,
       DontFindTopLevelAnchorInFileWithNoHTML_10Q) {
  const auto file_content_10Q = LoadDataFileForUse(FILE_WITH_NO_HTML_10Q);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);

  auto financial_content =
      FindFinancialContentUsingAnchors(&sections, FILE_WITH_NO_HTML_10Q);
...
TEST_F(FindAnchorsForFinancialStatements, FindAnchorsComplexHTML_NO_HREFS_10K) {
  const auto file_content_10K =
      LoadDataFileForUse(FILE_WITH_HTML_NO_HREFS1_10K);
  EM::FileContent file_content{file_content_10K};
  const auto sections = LocateDocumentSections(file_content);
  ...
  auto financial_content =
      FindFinancialContentUsingAnchors(&sections, FILE_WITH_HTML_NO_HREFS1_10K);
  ASSERT_FALSE(financial_content);
}
...
TEST_F(FindAnchorsForFinancialStatements, FindAnchorDestinations_10Q) {
  ...
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);
  auto financial_document = FindFinancialContentUsingAnchors(
      &sections, FILE_WITH_HTML_10Q_WITH_ANCHORS);
  ...
  auto balance_sheet_href = rng::find_if(anchors, [](const auto &anchor) {
    return AnchorFilterUsingRegex(regex_balance_sheet, anchor);
  });
  ...
  auto stmt_of_ops_href = rng::find_if(anchors, [](const auto &anchor) {
    return AnchorFilterUsingRegex(regex_operations, anchor);
  });
  ...
  auto cash_flows_href = rng::find_if(anchors, [](const auto &anchor) {
    return AnchorFilterUsingRegex(regex_cash_flow, anchor);
  });
...
TEST_F(FindIndividualFinancialStatements_10Q,
       FindBalanceSheetInFileWithHTML_10Q) {
  const auto file_content_10Q = LoadDataFileForUse(FILE_WITH_HTML_10Q);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);
  HTML_FromFile htmls{&sections, FILE_WITH_HTML_10Q};
  ...
  for (auto html : htmls) {
    if (document_filter(html)) {
      ...
      auto balance_sheet =
          std::find_if(tables.begin(), tables.end(), [](const auto &x) {
...
TEST_F(FindIndividualFinancialStatements_10Q,
       RangeFindBalanceSheetInFileWithHTML_10Q) {
  const auto file_content_10Q = LoadDataFileForUse(FILE_WITH_HTML_10Q);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);
  HTML_FromFile htmls{&sections, FILE_WITH_HTML_10Q};
  ...
  auto find_doc =
      rng::views::filter(document_filter) |
...
TEST_F(FindIndividualFinancialStatements_10Q,
       FindBalanceSheetWithAnchorsHTML5_10Q) {
  const auto file_content_10Q =
      LoadDataFileForUse(FILE_WITH_HTML_10Q_WITH_ANCHORS5);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);
  auto financial_content = FindFinancialContentUsingAnchors(
      &sections, FILE_WITH_HTML_10Q_WITH_ANCHORS5);
  ...
  auto balance_sheet = FindStatementContent<BalanceSheet>(
      financial_content->first, anchors, regex_balance_sheet,
      BalanceSheetFilter);
  ASSERT_TRUE(!balance_sheet.empty());
}

TEST_F(FindIndividualFinancialStatements_10Q,
       FindStatementOfOperationsWithAnchorsHTML5_10Q) {
  const auto file_content_10Q =
      LoadDataFileForUse(FILE_WITH_HTML_10Q_WITH_ANCHORS5);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);
  ...
  auto statement_of_operations = FindStatementContent<StatementOfOperations>(
      financial_content->first, anchors, regex_operations,
      StatementOfOperationsFilter);
  ASSERT_TRUE(!statement_of_operations.empty());
}

TEST_F(FindIndividualFinancialStatements_10Q,
       FindCashFlowsWithAnchorsHTML5_10Q) {
  const auto file_content_10Q =
      LoadDataFileForUse(FILE_WITH_HTML_10Q_WITH_ANCHORS5);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);
  ...
  auto cash_flows = FindStatementContent<CashFlows>(
      financial_content->first, anchors, regex_cash_flow, CashFlowsFilter);
...
TEST_F(FindIndividualFinancialStatements_10Q, FindCashFlowStatement2_10Q) {
  ...
  const auto file_content_10Q =
      LoadDataFileForUse(FILE_WITH_HTML_10Q_WITH_ANCHORS2);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);
  auto financial_content = FindFinancialContentUsingAnchors(
      &sections, FILE_WITH_HTML_10Q_WITH_ANCHORS2);
  EXPECT_TRUE(financial_content);
...
TEST_F(FindIndividualFinancialStatements_10Q,
       DISABLED_FindStockholderEquity_10Q) {
...
TEST_F(Multipliers, DISABLED_FindDollarMultipliers_10Q) {
  ...
  auto financial_content =
      FindFinancialContentUsingAnchors(&sections, FILE_WITH_HTML_ANCHORS_10Q);
...
TEST_F(Multipliers, FindDollarMultipliers_10Q) {
  ...
  auto financial_statements =
      ExtractFinancialStatements(financial_content->first);
...
TEST_F(ProcessEntireFile_10Q, ExtractAllNeededSections) {
  ...
  auto all_sections = FindAndExtractFinancialStatements(
      so, &sections, {"10-Q"}, FILE_WITH_HTML_10Q_WITH_ANCHORS);
...
TEST_F(ProcessEntireFile_10Q, ExtractAllNeededSections2) {
  ...
  const SharesOutstanding so;
  auto all_sections = FindAndExtractFinancialStatements(
      so, &sections, {"10-Q"}, FILE_WITH_HTML_10Q_WITH_ANCHORS2);
  std::cout << "\n\nBalance Sheet\n";
  ...
  std::cout << "\n\nStmt of Operations\n";
  std::cout.write(all_sections.statement_of_operations_.parsed_data_.data(),
                  500);
  std::cout << "\n\nShareholder Equity\n";
  std::cout.write(
      all_sections.stockholders_equity_.parsed_data_.data(),
...
TEST_F(ProcessEntireFile_10Q, ExtractAllNeededSections3) {
  const auto file_content_10Q =
      LoadDataFileForUse(FILE_WITH_HTML_10Q_WITH_ANCHORS3);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);
  FinancialDocumentFilter document_filter{{"10-Q"}};
...
TEST_F(ProcessEntireFile_10Q, ExtractAllNeededSections4) {
  const auto file_content_10Q =
      LoadDataFileForUse(FILE_WITH_HTML_10Q_WITH_ANCHORS4);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);
  auto financial_content = FindFinancialContentUsingAnchors(
      &sections, FILE_WITH_HTML_10Q_WITH_ANCHORS4);
  EXPECT_TRUE(financial_content);

  auto all_sections =
      ExtractFinancialStatementsUsingAnchors(financial_content->first);
  std::cout << "\n\nBalance Sheet\n";
  std::cout.write(all_sections.balance_sheet_.parsed_data_.data(), 500);
  std::cout << "\n\nCash Flow\n";
  std::cout.write(all_sections.cash_flows_.parsed_data_.data(), 500);
...
TEST_F(ProcessEntireFile_10Q, ExtractAllNeededSectionsMinimalHTMLData) {
  ...
  const SharesOutstanding so;
  auto all_sections = FindAndExtractFinancialStatements(
      so, &sections, {"10-Q"}, FILE_WITH_HTML_10Q_MINIMAL_DATA);
...
TEST_F(ProblemFiles_10Q, FileWithMinimalData) {
  ...
  const auto sections = LocateDocumentSections(file_content);
  const SharesOutstanding so;
  auto financial_statements = FindAndExtractFinancialStatements(
      so, &sections, {"10-Q"}, FILE_WITH_HTML_10Q_MINIMAL_DATA);
...
TEST_F(ProblemFiles_10K, FileWithMinimalDataAndNoFileNme) {
  const auto file_content_10K =
      LoadDataFileForUse(FILE_WITH_NO_HTML_AND_NO_FILENAME_10K);
  EM::FileContent file_content{file_content_10K};
  const auto sections = LocateDocumentSections(file_content);
  const SharesOutstanding so;

  ASSERT_THROW(
      FindAndExtractFinancialStatements(so, &sections, {"10-K"},
                                        FILE_WITH_NO_HTML_AND_NO_FILENAME_10K),
      ExtractorException);
}
...
TEST_F(ProblemFiles_10K, DISABLED_FindSectionAnchors_10K) {
  ...
  const auto sections = LocateDocumentSections(file_content);
  auto financial_content =
      FindFinancialContentUsingAnchors(&sections, FILE_WITH_ANCHOR_LOOP);
  EXPECT_TRUE(financial_content);
...
TEST_F(NoAnchors_10Q, FindContentInFileWithNoAnchors1) {
  const auto file_content_10Q =
      LoadDataFileForUse(FILE_WITH_HTML_10Q_NO_USABLE_ANCHORS);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);
  auto financial_content = FindFinancialContentUsingAnchors(
      &sections, FILE_WITH_HTML_10Q_NO_USABLE_ANCHORS);
...
TEST_F(NoAnchors_10Q, FileWithNoAnchors2) {
  const auto file_content_10Q =
      LoadDataFileForUse(FILE_WITH_HTML_10Q_NO_USABLE_ANCHORS2);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);
  ...
  auto the_tables = FindAndExtractFinancialStatements(
      so, &sections, {"10-Q"}, FILE_WITH_HTML_10Q_NO_USABLE_ANCHORS2);
...
TEST_F(NoAnchors_10Q, FileWithNoAnchors3) {
  const auto file_content_10Q =
      LoadDataFileForUse(FILE_WITH_HTML_10Q_NO_USABLE_ANCHORS3);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);
  // let's see if we can find our data anyways
  ...
  const SharesOutstanding so;
  auto the_tables = FindAndExtractFinancialStatements(
      so, &sections, {"10-Q"}, FILE_WITH_HTML_10Q_NO_USABLE_ANCHORS3);
  ...
  std::cout << "\n\nShareholder Equity\n";
  std::cout.write(
      the_tables.stockholders_equity_.parsed_data_.data(),
...
TEST_F(ProblemWithRegexs_10Q, UseRegexProblemFile1) {
  const auto file_content_10Q =
      LoadDataFileForUse(FILE_WITH_HTML_10Q_PROBLEM_REGEX1);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);

  const SharesOutstanding so;
  auto the_tables = FindAndExtractFinancialStatements(
      so, &sections, {"10-Q"}, FILE_WITH_HTML_10Q_PROBLEM_REGEX1);
...
TEST_F(ProblemWithRegexs_10Q, DISABLED_UseRegexProblemFile2) {
  ...
  const auto file_content_10Q =
      LoadDataFileForUse(FILE_WITH_HTML_10Q_PROBLEM_REGEX2);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);
  const SharesOutstanding so;
  auto the_tables = FindAndExtractFinancialStatements(
      so, &sections, {"10-Q"}, FILE_WITH_HTML_10Q_PROBLEM_REGEX2);
...
TEST_F(ProcessEntireFileAndExtractData_10Q, HTML_10Q_FIND_SHARES1) {
  const auto file_content_10Q =
      LoadDataFileForUse(FILE_WITH_HTML_10Q_FIND_SHARES1);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);

  const SharesOutstanding so;
  auto all_sections = FindAndExtractFinancialStatements(
      so, &sections, {"10-Q"}, FILE_WITH_HTML_10Q_FIND_SHARES1);
  EXPECT_TRUE(all_sections.has_data());
...
TEST_F(ProcessEntireFileAndExtractData_10Q, HTML_10Q_ASSETS_PROBLEM1) {
  const auto file_content_10Q =
      LoadDataFileForUse(FILE_WITH_HTML_10Q_PROBLEM_WITH_ASSETS1);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);

  const SharesOutstanding so;
  auto all_sections = FindAndExtractFinancialStatements(
      so, &sections, {"10-Q"}, FILE_WITH_HTML_10Q_PROBLEM_WITH_ASSETS1);
...
TEST_F(ProcessEntireFileAndExtractData_10Q, HTML10QBadCashFlowValue) {
  const auto file_content_10Q =
      LoadDataFileForUse(FILE_WITH_BAD_CASH_FLOW_VALUE);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);

  const SharesOutstanding so;
  auto all_sections = FindAndExtractFinancialStatements(
      so, &sections, {"10-Q"}, FILE_WITH_BAD_CASH_FLOW_VALUE);
...
TEST_F(ProcessEntireFileAndExtractData_10Q, XML_10K_NoSharesOutstanding) {
  const auto file_content_10K =
      LoadDataFileForUse(FILE_WITH_XBRL_INSIDE_TXT_DOCUMENT);
  EM::FileContent file_content{file_content_10K};
  const auto sections = LocateDocumentSections(file_content);

  const SharesOutstanding so;
  auto all_sections = FindAndExtractFinancialStatements(
      so, &sections, {"10-K"}, FILE_WITH_XBRL_INSIDE_TXT_DOCUMENT);
...
TEST_F(ProcessEntireFileAndExtractData_10Q, HTML_10Q_DUPLICATE_LABEL_TEXT) {
  ...
  const auto file_content_10Q =
      LoadDataFileForUse(FILE_SHOWING_DUPLICATE_LABEL_TEXT);
  EM::FileContent file_content{file_content_10Q};
  const auto sections = LocateDocumentSections(file_content);

  const SharesOutstanding so;
  auto all_sections = FindAndExtractFinancialStatements(
      so, &sections, {"10-Q"}, FILE_SHOWING_DUPLICATE_LABEL_TEXT);
  ...
  for (const auto &[key, value] :
       all_sections.statement_of_operations_.values_) {
    std::cout << "\nkey: " << key << " value: " << value << '\n';
  }
}
...
TEST_F(ProcessEntireFileAndExtractData_10K, XML_10K_Collect1) {
  ...
  const SharesOutstanding so;
  auto all_sections = FindAndExtractFinancialStatements(
      so, &sections, {"10-K"}, FILE_WITH_HTML_NO_HREFS1_10K);
...
TEST_F(ExportHTML, HTML_10Q) {
  ...
  HTML_FromFile htmls2{&exported_sections,
                       EM::FileName{"/tmp/export_html.txt"}};
  auto reimported_content =
      std::find_if(std::begin(htmls2), std::end(htmls2), document_filter);
...
TEST_F(ExportHTML, VerifyExportAll_10Q) {
  ...
  auto hierarchy_converter = ConvertInputHierarchyToOutputHierarchy(
      SEC_DIRECTORY, EM::FileName{"/tmp/exported_html"});

  auto export_file(
      [&files_with_form, &hierarchy_converter](const auto &dir_ent) {
        if (dir_ent.status().type() == fs::file_type::regular) {
          ...
          for (const auto &html : htmls) {
            ...
            auto output_path_name = hierarchy_converter(
                EM::FileName{dir_ent.path()}, html.file_name_.get().string());
            ...
            exported_file.write(file_header.data(), file_header.size());
            exported_file.put('\n');
            exported_file.write(html.document_.get().data(),
                                html.document_.get().size());
            exported_file.close();
            break;
          }
        }
...
TEST_F(ExportHTML, VerifyCanProcessExportedHTML_10Q) {
  ...
  auto process_exported_data(
      [&document_filter, &processed_files](const auto &dir_ent) {
        if (dir_ent.status().type() == fs::file_type::regular) {
          auto file_content_10Q =
              LoadDataFileForUse(EM::FileName{dir_ent.path().c_str()});
          EM::FileContent file_content{file_content_10Q};
