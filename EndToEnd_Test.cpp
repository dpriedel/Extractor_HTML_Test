...
const fs::path FILE_WITH_XML_10Q{
    "/vol_DA/SEC/Archives/edgar/data/1460602/0001062993-13-005017.txt"};
const fs::path FILE_WITH_XML_10K{
    "/vol_DA/SEC/Archives/edgar/data/google-10k.txt"};
const fs::path FILE_WITHOUT_XML{
    "/vol_DA/SEC/Archives/edgar/data/841360/0001086380-13-000030.txt"};
const fs::path SEC_DIRECTORY{"/vol_DA/SEC/Archives/edgar/data"};
const fs::path FILE_NO_NAMESPACE_10Q{
    "/vol_DA/SEC/Archives/edgar/data/68270/0000068270-13-000059.txt"};
const fs::path BAD_FILE2{
    "/vol_DA/SEC/SEC_forms/1000180/10-K/0001000180-16-000068.txt"};
const fs::path NO_SHARES_OUT{
    "/vol_DA/SEC/SEC_forms/1023453/10-K/0001144204-12-017368.txt"};
const fs::path MISSING_VALUES_LIST{
    "../Extractor_XBRL_Test/missing_values_files.txt"};
const fs::path CRASH_IN_REGEX{
    "/vol_DA/SEC/Archives/edgar/data/1060409/0001211524-13-000254.txt"};
...
public:
  ...
  int CountRows() {
    ...
    auto row1 =
        trxn.exec("SELECT count(*) FROM unified_extracts.sec_bal_sheet_data")
            .one_row();
    auto row2 =
        trxn.exec("SELECT count(*) FROM unified_extracts.sec_stmt_of_ops_data")
            .one_row();
    auto row3 =
        trxn.exec("SELECT count(*) FROM unified_extracts.sec_cash_flows_data")
            .one_row();
    trxn.commit();
...
TEST_F(ExportHTML, ExportSingleFile) {
  ...
  std::vector<std::string> tokens{
      "the_program", "--log-level", "debug", "--form", "10-Q,10-K", "--mode",
...
TEST_F(ExportHTML, ExportHMTLFromDirectory) {
  ...
  std::vector<std::string> tokens{
      ...
      "HTML", "--log-path", "/tmp/test1.log",
      //		"--list", "./list_with_bad_file.txt",
      "--form-dir", SEC_DIRECTORY.string(), "--export-HTML-data",
...
TEST_F(ExportHTML, VerifyNoExportOfExistingFilesWhenReplaceIsNotSpecifed) {
  ...
  auto x1 =
      CollectLastModifiedTimesForFilesInDirectoryTree("/tmp/extracts/html");
...
public:
  ...
  void SetUp() override {
    ...
    trxn.exec(
        "UPDATE unified_extracts.sec_filing_id SET shares_outstanding = -1");
    trxn.commit();
  }
  int CountRows() {
    pqxx::connection c{"dbname=sec_extracts user=extractor_pg"};
...
TEST_F(TestBoth, UpdateDBFromList) {
  ...
  std::vector<std::string> tokens{"the_program", "--log-level", "info",
                                  "--form", "10-Q,10-K", "--mode", "BOTH",
                                  //		"-k", "6",
                                  "--list", "./test_directory_list.txt",
                                  "--log-path", "/tmp/test1.log"};
